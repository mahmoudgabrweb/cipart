...
class WeightController extends MainController
{
    public function __construct()
    {
        $this->moduleName = "weights";
        $this->model = Weight::class;
    }
    ...
    public function load()
    {
        ...
        return datatables()
            ...
            ->addColumn('actions', function ($row) use ($moduleName) {
                ...
                {
                    $buttons .="<a href='/admin/weights/$row->id/edit' class='btn btn-sm btn-warning'>Edit</a>";
                }

                if(auth()->user()->hasPermission("delete_$moduleName"))
                {
                    $buttons .="<a href='javascript:void(0);' data-url='/admin/weights/$row->id' class='delete-record btn btn-sm btn-danger'>Delete</a>";
                }
                return $buttons;
            })
            ->rawColumns(['actions'])
    ...
    public function store(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            'key' => 'required|string|unique:widths,key',
        ]);
    ...
    public function update(Request $request, int $id): RedirectResponse
    {
        $request->validate([
            'name_ar' => 'required|string',
            'name_en' => 'nullable|string',
            'key' => 'required|string|unique:widths,key,' . $id,
        ]);

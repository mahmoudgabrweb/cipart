...
class PackageWeightController extends MainController
{
    public function __construct()
    {
        $this->moduleName = "package_weights";
        $this->model = PackageWeight::class;
    }
    ...
    public function load()
    {
        ...
        return datatables()
            ...
            ->addColumn('actions', function ($row) use ($moduleName) {
                ...
                {
                    $buttons .="<a href='/admin/package_weights/$row->id/edit' class='btn btn-sm btn-warning'>Edit</a>";
                }

                if(auth()->user()->hasPermission("delete_$moduleName"))
                {
                    $buttons .="<a href='javascript:void(0);' data-url='/admin/package_weights/$row->id' class='delete-record btn btn-sm btn-danger'>Delete</a>";
                }
                return $buttons;
            })
            ->rawColumns(['actions'])

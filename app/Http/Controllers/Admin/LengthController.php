...
class LengthController extends MainController
{
    public function __construct()
    {
        $this->moduleName = "lengths";
        $this->model = Length::class;
    }
    ...
    public function load()
    {
        ...
        return datatables()
            ...
            ->addColumn('actions', function ($row) use ($moduleName) {
                ...
                {
                    $buttons .="<a href='/admin/lengths/$row->id/edit' class='btn btn-sm btn-warning'>Edit</a>";
                }

                if(auth()->user()->hasPermission("delete_$moduleName"))
                {
                    $buttons .="<a href='javascript:void(0);' data-url='/admin/lengths/$row->id' class='delete-record btn btn-sm btn-danger'>Delete</a>";
                }
                return $buttons;
            })
            ->rawColumns(['actions'])
    ...
    public function store(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            'key' => 'required|string|unique:lengths,key',
        ]);
    ...
    public function update(Request $request, int $id): RedirectResponse
    {
        $request->validate([
            'name_ar' => 'required|string',
            'name_en' => 'nullable|string',
            'key' => 'required|string|unique:lengths,key,' . $id,
        ]);

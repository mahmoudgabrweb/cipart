...
class WidthController extends MainController
{
    public function __construct()
    {
        $this->moduleName = "widths";
        $this->model = Width::class;
    }
    ...
    public function load()
    {
        ...
        return datatables()
            ...
            ->addColumn('actions', function ($row) use ($moduleName) {
                ...
                {
                    $buttons .="<a href='/admin/widths/$row->id/edit' class='btn btn-sm btn-warning'>Edit</a>";
                }

                if(auth()->user()->hasPermission("delete_$moduleName"))
                {
                    $buttons .="<a href='javascript:void(0);' data-url='/admin/widths/$row->id' class='delete-record btn btn-sm btn-danger'>Delete</a>";
                }
                return $buttons;
            })
            ->rawColumns(['actions'])
